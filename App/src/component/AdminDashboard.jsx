...
const DonorDashboard = () => {
    ...
    useEffect(() => {
        (async () => {
            try {
                const result = await axiosInstance.get(`admin-dashboard-stat`);
        ...
        <ScrollView className="bg-gray-100 py-5">
            <View className="px-4">
                ...
                <Card
                    ...
                    value={inform.AdminReceived}
                    onPress={() => navigation.navigate('Total Completed Donation')}
                />

                <Card
                    icon={<MaterialIcons name="pending-actions" size={24} color="#f43f5e" />}
                    label="Total Pending Request"
                    value={inform.AdminPending}
                    onPress={() => navigation.navigate(`New Pending Request`)}
                />

                <Card
                    icon={<Entypo name="check" size={24} color="#3b82f6" />}
                    label="Total On Going Donation"
                    value={inform.AdminAccepted}
                    onPress={() => navigation.navigate('On Going Donation')}
                />
            </View>
        </ScrollView>
    );
