...
const DonorOnGoing = () => {
    ...
    useEffect(() => {
        (async () => {
            try {
                const donationList = await axiosInstance.get(`donor/accept-donation`);
    ...
    return (
        <ScrollView className="bg-gray-100 pt-5 px-4 ">
            ...
            {donations.length === 0 ? (
                <Text className="text-center text-gray-500 italic">No Ongoing Events</Text>
            ) : (
                donations.map((item) => (
                    <View key={item._id} className="bg-white rounded-md shadow p-4 mb-4 border-2 border-orange-300">
                        ...
                        <Text className="text-gray-600 mb-1">🎁 {item.DonationType}</Text>
