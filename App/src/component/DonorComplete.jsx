...
const DonorComplete = () => {
    ...
    useEffect(() => {
        (async () => {
            try {
                const donationList = await axiosInstance.get(`donor/received-donation`);
    ...
    return (
        <ScrollView className="bg-gray-100 pt-5 px-4">
            ...
            {donations.length === 0 ? (
                <Text className="text-center text-gray-500 italic">No Completed Events</Text>
            ) : (
                donations.map((item) => (
